...
import {
  ...
  getFinanceSummary, getFinanceSources, getFinanceAccounts, getFinanceReceipts, getFinanceAllocations,
  createFinanceAccount, updateFinanceAccount, deleteFinanceAccount,
  createFinanceSource, updateFinanceSource, deleteFinanceSource,
  createFinanceReceipt, updateFinanceReceipt, deleteFinanceReceipt,
  createFinanceAlloc, updateFinanceAlloc, deleteFinanceAlloc,
} from '../../api/client'
...
const ACCOUNT_COLORS = ['#00A2FF', '#00CC88', '#FFB347', '#FF6B6B', '#A78BFA', '#34D399']

function FinanceForm({ title, fields, initial = {}, onSave, onCancel }) {
  const [form, setForm] = useState(initial)
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))
  return (
    <div className="bg-[#0F0F23] border border-[#2D2B5A] rounded-2xl p-4 space-y-3">
      <p className="text-white font-semibold text-sm">{title}</p>
      {fields.map(f => (
        <div key={f.key}>
          <label className="text-[#8892B0] text-xs">{f.label}</label>
          {f.type === 'select' ? (
            <select value={form[f.key] ?? f.default ?? ''} onChange={e => set(f.key, e.target.value)}
              className="w-full bg-[#16213E] border border-[#2D2B5A] rounded-xl px-3 py-2 text-white text-sm mt-1 focus:outline-none focus:border-[#00A2FF]">
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={() => onSave(form)} className="px-4 py-2 bg-[#00A2FF] text-white rounded-xl text-sm font-semibold">Save</button>
        <button onClick={onCancel} className="px-4 py-2 bg-[#2D2B5A] text-[#8892B0] rounded-xl text-sm">Cancel</button>
      </div>
    </div>
  )
}

function FinanceSection({ icon, title, children, onAdd }) {
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs text-[#8892B0] font-semibold">{icon} {title}</p>
        {onAdd && (
          <button onClick={onAdd} className="text-xs text-[#00A2FF] hover:text-white font-semibold">+ Add</button>
        )}
      </div>
      {children}
    </div>
...
function FinanceTab() {
  ...
  // which add-form is open: 'account' | 'source' | 'receipt' | 'alloc' | null
  ...
  const load = useCallback(async () => {
    try {
      const [fRes, sRes, rRes, aRes] = await Promise.all([
        getFinanceSummary(), getFinanceSources(), getFinanceReceipts(), getFinanceAllocations(),
      ])
      setFin(fRes.data)
  ...
  async function saveAccount(form) {
    if (editItem?.type === 'account') {
      await updateFinanceAccount(editItem.item.id, form)
    } else {
      await createFinanceAccount(form)
    }
    setAddForm(null); setEditItem(null); load()
  }

  async function saveSource(form) {
    if (editItem?.type === 'source') {
      await updateFinanceSource(editItem.item.id, form)
    } else {
      await createFinanceSource(form)
    }
    setAddForm(null); setEditItem(null); load()
  }
  ...
  async function saveReceipt(form) {
    if (editItem?.type === 'receipt') {
      await updateFinanceReceipt(editItem.item.id, form)
    } else {
      await createFinanceReceipt(form)
    }
    setAddForm(null); setEditItem(null); load()
  }

  async function saveAlloc(form) {
    if (editItem?.type === 'alloc') {
      await updateFinanceAlloc(editItem.item.id, form)
    } else {
      await createFinanceAlloc(form)
    }
    setAddForm(null); setEditItem(null); load()
  }

  async function del(type, id) {
    if (type === 'account') await deleteFinanceAccount(id)
    else if (type === 'source') await deleteFinanceSource(id)
    else if (type === 'receipt') await deleteFinanceReceipt(id)
    else if (type === 'alloc') await deleteFinanceAlloc(id)
    load()
  }

  const today = new Date().toISOString().slice(0, 10)
  const sourceOptions = sources.map(s => ({ value: s.id, label: s.name }))
  const accountOptions = (fin?.accounts || []).map(a => ({ value: a.id, label: a.name }))

  if (loading) return <div className="flex justify-center py-16"><div className="w-8 h-8 border-2 border-[#00A2FF] border-t-transparent rounded-full animate-spin" /></div>

  const activeForm = editItem ? editItem.type : addForm

  return (
    <div className="space-y-8">

      {/* ── Tier 0: Bank / Cash Balances ── */}
      <FinanceSection icon="🏦" title="BANK / CASH ACCOUNTS"
        onAdd={() => { setEditItem(null); setAddForm('account') }}>
        {(activeForm === 'account') && (
          <FinanceForm
            title={editItem ? 'Edit Account' : 'New Account'}
            initial={editItem?.item || { type: 'bank', opening_balance: 0 }}
            fields={[
              { key: 'name', label: 'Account Name', placeholder: 'HDFC Savings' },
              { key: 'type', label: 'Type', type: 'select', options: [{ value: 'bank', label: 'Bank' }, { value: 'cash', label: 'Cash' }] },
              { key: 'opening_balance', label: 'Opening Balance (₹)', type: 'number' },
              { key: 'notes', label: 'Notes', placeholder: 'Optional' },
            ]}
            onSave={saveAccount}
            onCancel={() => { setAddForm(null); setEditItem(null) }}
          />
        )}
        {fin?.accounts?.length === 0 && !activeForm && (
          <p className="text-[#8892B0] text-sm">No accounts yet. Add a bank or cash account.</p>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {(fin?.accounts || []).map((acc, i) => (
            <div key={acc.id} className="bg-[#16213E] border border-[#2D2B5A] rounded-2xl p-4 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-lg">{acc.type === 'cash' ? '💵' : '🏦'}</span>
                  <div>
                    <p className="text-white font-semibold text-sm">{acc.name}</p>
                    <p className="text-[#8892B0] text-xs capitalize">{acc.type}</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => { setEditItem({ type: 'account', item: acc }); setAddForm(null) }}
                    className="text-xs text-[#8892B0] hover:text-[#00A2FF]">Edit</button>
                  <button onClick={() => del('account', acc.id)} className="text-xs text-[#8892B0] hover:text-[#FF3333]">✕</button>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center pt-1">
                <div>
                  <p className="text-[#8892B0] text-[10px]">Received</p>
                  <p className="text-[#00CC88] text-sm font-bold">{fmtINR(acc.total_received)}</p>
                </div>
                <div>
                  <p className="text-[#8892B0] text-[10px]">Spent</p>
                  <p className="text-[#FFB347] text-sm font-bold">{fmtINR(acc.total_spent)}</p>
                </div>
                <div>
                  <p className="text-[#8892B0] text-[10px]">Balance</p>
                  <p className={`text-sm font-bold ${acc.balance < 0 ? 'text-[#FF3333]' : 'text-white'}`}>{fmtINR(acc.balance)}</p>
                </div>
              </div>
              {/* balance bar */}
              {(acc.total_received + acc.opening_balance) > 0 && (
                <div className="h-1 bg-[#0F0F23] rounded-full overflow-hidden">
                  <div className="h-full bg-[#00A2FF] rounded-full"
                    style={{ width: `${Math.min(100, (acc.balance / (acc.opening_balance + acc.total_received)) * 100)}%` }} />
                </div>
              )}
            </div>
          ))}
        ...
        {fin?.accounts?.length > 0 && (
          <div className="bg-[#16213E] border border-[#00A2FF]/30 rounded-2xl p-4 flex justify-between items-center">
            <p className="text-[#8892B0] text-sm font-semibold">Total Balance Across All Accounts</p>
            <p className={`text-xl font-fredoka font-bold ${fin.total_bank_balance < 0 ? 'text-[#FF3333]' : 'text-[#00A2FF]'}`}>
              {fmtINR(fin.total_bank_balance)}
            </p>
          </div>
        )}
      </FinanceSection>
      ...
      {/* ── Tier 1: Fund Sources ── */}
      <FinanceSection icon="💰" title="TIER 1 — FUND SOURCES"
        onAdd={() => { setEditItem(null); setAddForm('source') }}>
        {activeForm === 'source' && (
          <FinanceForm
            title={editItem ? 'Edit Source' : 'New Fund Source'}
            initial={editItem?.item || { type: 'loan' }}
            fields={[
              { key: 'name', label: 'Source Name', placeholder: 'HDFC Home Loan' },
              { key: 'type', label: 'Type', type: 'select', options: [
                { value: 'loan', label: 'Loan' }, { value: 'savings', label: 'Savings' },
                { value: 'income', label: 'Income' }, { value: 'other', label: 'Other' },
              ]},
              { key: 'total_sanctioned', label: 'Total Sanctioned (₹)', type: 'number', placeholder: '5000000' },
              { key: 'notes', label: 'Notes', placeholder: 'Optional' },
            ]}
            onSave={saveSource}
            onCancel={() => { setAddForm(null); setEditItem(null) }}
        ...
        {sources.length === 0 && !activeForm && (
          <p className="text-[#8892B0] text-sm">No fund sources yet.</p>
        )}
        <div className="space-y-2">
          {sources.map(s => (
            <div key={s.id} className="bg-[#16213E] border border-[#2D2B5A] rounded-xl px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-semibold">{s.name}
                  <span className="ml-2 text-xs text-[#8892B0] capitalize">{s.type}</span>
                </p>
                <div className="flex gap-4 mt-1">
                  {s.total_sanctioned > 0 && <span className="text-[#8892B0] text-xs">Sanctioned: <span className="text-white font-semibold">{fmtINR(s.total_sanctioned)}</span></span>}
                  <span className="text-[#8892B0] text-xs">Received: <span className="text-[#00CC88] font-semibold">{fmtINR(s.total_received)}</span></span>
                </div>
                {s.total_sanctioned > 0 && (
                  <div className="h-1 bg-[#0F0F23] rounded-full mt-2 overflow-hidden">
                    <div className="h-full bg-[#00CC88] rounded-full"
                      style={{ width: `${Math.min(100, (s.total_received / s.total_sanctioned) * 100)}%` }} />
                  </div>
                )}
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => { setEditItem({ type: 'source', item: s }); setAddForm(null) }}
                  className="text-xs text-[#8892B0] hover:text-[#00A2FF]">Edit</button>
                <button onClick={() => del('source', s.id)} className="text-xs text-[#8892B0] hover:text-[#FF3333]">✕</button>
              </div>
            </div>
          ))}
        </div>
        {/* Total received vs sanctioned */}
        {fin && (
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-[#16213E] border border-[#2D2B5A] rounded-xl p-3 text-center">
              <p className="text-[#8892B0] text-[10px] font-semibold">SANCTIONED</p>
              <p className="text-white font-fredoka font-bold text-base mt-1">{fmtINR(fin.total_sanctioned)}</p>
            </div>
            <div className="bg-[#16213E] border border-[#2D2B5A] rounded-xl p-3 text-center">
              <p className="text-[#8892B0] text-[10px] font-semibold">RECEIVED</p>
              <p className="text-[#00CC88] font-fredoka font-bold text-base mt-1">{fmtINR(fin.total_received)}</p>
            </div>
            <div className="bg-[#16213E] border border-[#2D2B5A] rounded-xl p-3 text-center">
              <p className="text-[#8892B0] text-[10px] font-semibold">UNALLOCATED</p>
              <p className={`font-fredoka font-bold text-base mt-1 ${fin.unallocated < 0 ? 'text-[#FF3333]' : 'text-[#00A2FF]'}`}>{fmtINR(fin.unallocated)}</p>
            </div>
          </div>
        )}
      </FinanceSection>
      ...
      {/* ── Fund Receipts ── */}
      <FinanceSection icon="📥" title="FUND RECEIPTS (TRANCHES)"
        onAdd={() => { setEditItem(null); setAddForm('receipt') }}>
        {activeForm === 'receipt' && (
          <FinanceForm
            title={editItem ? 'Edit Receipt' : 'New Receipt'}
            initial={editItem?.item || { date: today }}
            fields={[
              { key: 'source_id', label: 'Source', type: 'select', options: sourceOptions },
              { key: 'account_id', label: 'Into Account (optional)', type: 'select', options: [{ value: '', label: '— None —' }, ...accountOptions] },
              { key: 'amount', label: 'Amount (₹)', type: 'number' },
              { key: 'date', label: 'Date', type: 'date' },
              { key: 'ref_number', label: 'Reference / UTR', placeholder: 'Optional' },
              { key: 'notes', label: 'Notes', placeholder: 'Optional' },
            ]}
            onSave={saveReceipt}
            onCancel={() => { setAddForm(null); setEditItem(null) }}
        ...
        {receipts.length === 0 && !activeForm && (
          <p className="text-[#8892B0] text-sm">No receipts yet.</p>
        )}
        <div className="space-y-2">
          {receipts.map(r => (
            <div key={r.id} className="bg-[#16213E] border border-[#2D2B5A] rounded-xl px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <p className="text-[#00CC88] font-bold text-sm">+{fmtINR(r.amount)}</p>
                  <span className="text-[#8892B0] text-xs">{fmt(r.date)}</span>
                </div>
                <p className="text-[#8892B0] text-xs mt-0.5">
                  {r.source_name}
                  {r.account_name && <> → <span className="text-white">{r.account_name}</span></>}
                  {r.ref_number && <> · {r.ref_number}</>}
                </p>
              </div>
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => { setEditItem({ type: 'receipt', item: { ...r, source_id: r.source_id, account_id: r.account_id ?? '' } }); setAddForm(null) }}
                  className="text-xs text-[#8892B0] hover:text-[#00A2FF]">Edit</button>
                <button onClick={() => del('receipt', r.id)} className="text-xs text-[#8892B0] hover:text-[#FF3333]">✕</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── Tier 2: Category Allocations ── */}
      <FinanceSection icon="📊" title="TIER 2 — CATEGORY ALLOCATIONS"
        onAdd={() => { setEditItem(null); setAddForm('alloc') }}>
        {activeForm === 'alloc' && (
          <FinanceForm
            title={editItem ? 'Edit Allocation' : 'New Allocation'}
            initial={editItem?.item || { date: today }}
            fields={[
              { key: 'category', label: 'Category', placeholder: 'PolyHouse / Flat / Personal' },
              { key: 'amount', label: 'Amount (₹)', type: 'number' },
              { key: 'source_id', label: 'From Source (optional)', type: 'select', options: [{ value: '', label: '— None —' }, ...sourceOptions] },
              { key: 'date', label: 'Date', type: 'date' },
              { key: 'notes', label: 'Notes', placeholder: 'Optional' },
            ]}
            onSave={saveAlloc}
        ...
        {allocs.length === 0 && !activeForm && (
          <p className="text-[#8892B0] text-sm">No allocations yet.</p>
        )}
        <div className="space-y-2">
          {allocs.map(a => (
            <div key={a.id} className="bg-[#16213E] border border-[#2D2B5A] rounded-xl px-4 py-3 flex items-center gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-white text-sm font-semibold">{a.category}
                  {a.source_name && <span className="text-[#8892B0] text-xs font-normal ml-2">from {a.source_name}</span>}
                </p>
                <p className="text-[#8892B0] text-xs mt-0.5">{fmt(a.date)}{a.notes && ` · ${a.notes}`}</p>
              </div>
              <p className="text-white font-bold text-sm flex-shrink-0">{fmtINR(a.amount)}</p>
              <div className="flex gap-2 flex-shrink-0">
                <button onClick={() => { setEditItem({ type: 'alloc', item: { ...a, source_id: a.source_id ?? '' } }); setAddForm(null) }}
                  className="text-xs text-[#8892B0] hover:text-[#00A2FF]">Edit</button>
                <button onClick={() => del('alloc', a.id)} className="text-xs text-[#8892B0] hover:text-[#FF3333]">✕</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── Tier 3 summary: allocation vs spending per category ── */}
      {fin?.categories?.length > 0 && (
        <FinanceSection icon="🏗️" title="TIER 3 — ALLOCATION vs SPENDING">
          <div className="space-y-3">
            {fin.categories.map(c => (
              <div key={c.category} className="bg-[#16213E] border border-[#2D2B5A] rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-white font-fredoka font-bold">{c.category}</p>
                  <p className={`text-sm font-bold ${c.balance < 0 ? 'text-[#FF3333]' : 'text-[#00CC88]'}`}>
                    {c.balance < 0 ? '−' : '+'}{fmtINR(Math.abs(c.balance))} left
                  </p>
                </div>
                <div className="grid grid-cols-3 gap-3 text-center">
                  <div>
                    <p className="text-[#8892B0] text-[10px]">ALLOCATED</p>
                    <p className="text-white font-semibold text-sm">{fmtINR(c.allocated)}</p>
                  </div>
                  <div>
                    <p className="text-[#8892B0] text-[10px]">SPENT (PAID)</p>
                    <p className="text-[#FFB347] font-semibold text-sm">{fmtINR(c.spent)}</p>
                  </div>
                  <div>
                    <p className="text-[#8892B0] text-[10px]">COMMITTED</p>
                    <p className="text-[#00A2FF] font-semibold text-sm">{fmtINR(c.committed)}</p>
                  </div>
                </div>
                {c.allocated > 0 && (
                  <div className="h-2 bg-[#0F0F23] rounded-full overflow-hidden flex">
                    <div className="h-full bg-[#FFB347] rounded-l-full" style={{ width: `${Math.min(100, (c.spent / c.allocated) * 100)}%` }} />
                    <div className="h-full bg-[#00A2FF]" style={{ width: `${Math.min(100 - Math.min(100, (c.spent / c.allocated) * 100), (c.committed / c.allocated) * 100)}%` }} />
                  </div>
                )}
                <p className="text-[#8892B0] text-xs">
                  {c.allocated > 0
                    ? `${((c.spent / c.allocated) * 100).toFixed(1)}% paid · ${((c.committed / c.allocated) * 100).toFixed(1)}% committed`
                    : 'No allocation — spending from tasks only'}
                </p>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex gap-1 bg-[#16213E] border border-[#2D2B5A] rounded-2xl p-1 w-fit">
        {[['overview', '📊 Overview'], ['tasks', '📋 Tasks'], ['expenses', '💰 Expenses'], ['finance', '🏦 Finance']].map(([id, label]) => (
